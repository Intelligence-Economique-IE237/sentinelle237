...
const SECTION_META: Record<SelectedView["type"], SectionMetaEntry> = {
  ...
  // Reste dans SelectedView["type"] pour la sidebar partagée, mais n'est
  // jamais rendu ici — le clic est intercepté et redirige vers
  // /tableau-de-bord (UserDashboardView, page autonome avec sa propre
  // sidebar). Voir le onSelect de <AppSidebar> plus bas.
  dashboard: {
    icon: Gauge,
  ...
  return (
      <SidebarProvider className="h-svh" style={{ "--sidebar-width": "350px" } as React.CSSProperties}>
        <AppSidebar
            selected={selected}
            onSelect={(view) => {
              // "Tableau de bord" vit maintenant sur sa propre route/page
              // (UserDashboardView, avec sa propre sidebar) — on redirige au
              // lieu d'essayer de le rendre ici.
